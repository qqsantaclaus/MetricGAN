...
from utils import read_batch_STOI, read_batch_PESQ, read_batch_PESQ_py
...
reload_model ='./models_ckpt_400/current_SE_model.h5'
...
for (dirpath, dirnames, filenames) in os.walk(data_root_path):
    ...
    if sp_env not in sp_envs_results:
        sp_envs_results[sp_env] = {"PESQ":[], "STOI":[]}
    ref_filename_list = []
    ...
    for filename in filenames:
        ...
        if not filename in env_sel:
            continue
        enhanced_name = os.path.join(output_path, reload_model.split("/")[-1].split(".")[0], sp_env, "MetricGAN", filename.replace("Reverb", "MetricGAN"))
        creatdir(os.path.join(output_path, reload_model.split("/")[-1].split(".")[0], sp_env, "MetricGAN"))
        ...
        ref_filename = os.path.join(dirpath, filename).replace("Reverb", "Clean")

        ref_filename_list.append(ref_filename)
        enhanced_filename_List.append(enhanced_name)
    # Calculate True STOI    
    test_STOI=read_batch_STOI(ref_filename_list, enhanced_filename_List)     
    sp_envs_results[sp_env]["STOI"] = test_STOI
    # Calculate True PESQ    
    test_PESQ=read_batch_PESQ_py(ref_filename_list, enhanced_filename_List)     
    sp_envs_results[sp_env]["PESQ"] = test_PESQ
    
    print(sp_env, np.mean(sp_envs_results[sp_env]["STOI"]), np.mean(sp_envs_results[sp_env]["PESQ"]))

end_time = time.time()
print('The code for this file ran for %.2fm' % ((end_time - start_time) / 60.))
print("STOI:", np.mean([sp_envs_results[key]["STOI"] for key in sp_envs_results.keys()]), 
      "PESQ:", np.mean([sp_envs_results[key]["PESQ"] for key in sp_envs_results.keys()]))