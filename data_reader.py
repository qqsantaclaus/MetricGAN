    ...
    def __iter__(self):
        ...
        while True:
            for idx in self.indexes:
                ...
                if self.seq_len > 0:
                    cut = int(np.random.randint(0, len(speech_audio_1)-int(1.2*self.seq_len), 1))
                    speech_audio_1 = speech_audio_1[cut:cut+int(1.2*self.seq_len)]
                ...
                if self.seq_len > 0:
                    new_speech_audio = new_speech_audio[:self.seq_len]
                    noisy_audio = noisy_audio[:self.seq_len]
